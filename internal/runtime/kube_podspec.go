...
// hostPath mode; freely scheduled in csi mode). HelperSpec.Network is ignored
// on K8s: the pod network reaches both cluster pods and external hosts, which
// is all helpers need. HelperSpec.HostDevices is also ignored: a privileged
// container sees host devices already.
func buildHelperPod(namespace string, st kubeStorage, spec HelperSpec) *corev1.Pod {
...
// schedule anywhere with no extra capabilities.
